...
// **write this function**
  uint32_t address = (sector*512) + EDISK_ADDR_MIN;
  if(address > EDISK_ADDR_MAX){
    return RES_PARERR;
	}
  uint8_t *pt = (uint8_t *)address;
...
// **write this function**
  ...
  uint32_t addr = (sector*512) + EDISK_ADDR_MIN;
  if(addr > EDISK_ADDR_MAX){
    return RES_PARERR;
  }
  if(Flash_WriteArray(buff_ptr, addr, 128) == ERROR){
      return RES_ERROR;
  }
...
// **write this function**
  for(uint32_t rom_addr = EDISK_ADDR_MIN; rom_addr < EDISK_ADDR_MAX; rom_addr += 1024){
    Flash_Erase(rom_addr);
  }
  return RES_OK;
}
